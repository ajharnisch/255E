...
#inster r code into rchunk 
```{r tideFunction}
# coops_tide_ht_retrieval.R
# Author: Luke Miller Feb 2011
# Updated 2017-09-26 to handle newer ERDDAP format

#note function resres = NOAAfunction(station = 9413450,startDate = 20120101, endDate - 20120128)
################################################################################
...
library(RCurl)
# you can also use requires function or etc. 
...
#do not need to ask user to enter
...
#			xlab = "Date",ylab = "Tide Height, meters")
	...
	# you can comment out this section goal is to generate new plots of data
#	filename = paste("Station_",station,"_",startdate,"-",enddate,
	#		".csv",sep = "") #make file name
#	write.csv(df,filename,row.names = FALSE, quote = FALSE) #write file to disk note this should be deleted. 
...
invisible(df) #all of df will be spit out 
} #closing bracket for the funciton defination NOAA function 
...
plotTides <- function(df){
  plot(x=df$TimeUTC, y= df$TideHT, type='l', xlab = 'Date', ylab = 'TideHt, m', col = 'blue')
}
```


cash directories when using the knit function use rstudio to form word document. inorder to avoid this. store the results for each of the chunks, r will change results. avoid continuous set of data

```{r grabJan, cache=TRUE, message = FALSE, warning = FALSE}
jan = NOAAfunction(9413450, 20160101, 20160201)
```
output messages can be displayed,  



changing the global cache conflict, deleting the cache folder can eliminate conflicts, sometimes this can occur because of the when chunk for cashe import, leave the plot diffingation unchunked. when kniting, the changes will propogate in each plot. 
