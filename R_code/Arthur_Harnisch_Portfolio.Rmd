...
annotate("text", x=9, y=.6, label = "25uM NE")+
...
theme(plot.title = element_text(hjust=0.5))+
  scale_x_continuous("Ramp", breaks = round(seq(min(Exp$ramp.num), max(Exp$ramp.num), by=2),1))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_y_continuous("Percent change from baseline", labels = scales::percent, breaks = round(seq(min(Exp$pvb.peak.tension), max(Exp$pvb.peak.tension), by = 0.1),1))+theme(legend.position = "none")+
annotate("text", x=9, y=.6, label = "25uM NE")+
  annotate("text", x=31, y=.8, label = "10uM NE")
...
c1 = ggplot(Exp, aes(x=ramp.num, y=pvb.SR, color= Trial))+
  ...
  scale_x_continuous("Ramp",breaks = round(seq(min(Exp$ramp.num), max(Exp$ramp.num), by=2),1))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_y_continuous("Percent change from baseline", labels = scales::percent, breaks = round(seq(min(Exp$pvb.SR), max(Exp$pvb.SR), by = 0.1),1))+theme(legend.position = "none")+
annotate("text", x=9, y=.6, label = "25uM NE")+
  annotate("text", x=31, y=.8, label = "10uM NE")
