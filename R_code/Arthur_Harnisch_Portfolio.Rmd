...
#png
#install.packages("png")# did not use but thought helpful
...
g <- rasterGrob(img, interpolate=TRUE) 
 ...
 ggplot(Exp, aes(x= ramp.num, y= pvb.BL.tension, color = Trial))+
  geom_point()+
   geom_line()+
 #annotation_custom(g, xmin=-Inf, xmax=Inf, ymin=min(Exp$pvb.BL.tension), ymax=Inf)
 annotation_custom(g, xmin=-Inf, xmax=Inf, ymin= max(Exp$pvb.BL.tension) , ymax=Inf)+
   ylim(min(Exp$pvb.BL.tension), 2)+
   geom_smooth(method ="lm", se=TRUE, color = "black", aes (group = 1))+
   theme_minimal()+
    scale_x_continuous("Time(Min.)", breaks = round(seq(min = maxExp$ramp.num, max(Exp$ramp.num), by=2),1))+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  scale_y_continuous("Percent Variation from Baseline", breaks = round(seq(min(Exp$pvb.BL.tension), max(Exp$pvb.BL.tension), by=0.01),1))
...
Exp_20 =as.data.frame(Exp$BL.tension)
Exp_20$Trial = Exp$Trial
Exp_20$Macro = Exp$Macro
Exp_BL.tension = Exp$BL.tension
Exp_20$ramp.num = Exp$ramp.num

Exp_19 = melt(Exp_20, id.vars=c('Trial', 'ramp.num', 'Exp$BL.tension'))
dcast(Exp_19, Trial + Exp$BL.tension ~ ramp.num)



Exp_19 = Exp_20 %>%
  group_by(ramp.num)%>%
  summarise(avg_BSL = mean(BL.tension), na.rm = TRUE)
...
write.xlsx(Exp12, file = "C:\\Users\\Arthur\\255E\\Tension_R\\Exp_output\\Exp_out.xlsx", sheetName="Sheet1")
