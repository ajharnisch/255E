...
summarise(avg_DP = mean(DP, na.rm = TRUE))
 ...
 Control$avg_DP = con3$avg_avg_DP[match(Control$Trial, con3$Trial)]
 ...
 Control$avg_IST = con4$avg_IST[match(Control$Trial, con2$Trial)]
...
ggplot(Exp, aes(x= ramp.num, y= pcb.resting.discharge, color = Trial)) +
  ...
  scale_y_continuous("Percent change from baseline",labels = scales::percent, breaks = round(seq(min(Exp$pcb.resting.discharge), max(Exp$pcb.resting.discharge), by = 0.1),1)) 
...
ggplot(Exp, aes(x= ramp.num, y= pcb.50hz, color = Trial)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_y_continuous("Percent change from baseline",labels = scales::percent, breaks = round(seq(min(Exp$pcb.50hz), max(Exp$pcb.50hz), by = 0.1),1)) 
...
ggplot(Exp, aes(x= ramp.num, y= pcb.100hz, color = Trial)) +
  ...
  scale_x_continuous("Vibration number", breaks = round(seq(min(Exp$ramp.num), max(Exp$ramp.num), by=2),1))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  scale_y_continuous("Percent change from baseline",labels = scales::percent, breaks = round(seq(min(Exp$pcb.100hz), max(Exp$pcb.100hz), by = 0.1),1)) 
